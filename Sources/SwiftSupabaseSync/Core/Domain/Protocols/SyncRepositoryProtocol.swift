...
public extension SyncRepositoryProtocol {
    
    /// Default implementation for cleanup with 30-day threshold
    func cleanupSyncMetadata() async throws {
        let thirtyDaysAgo = Calendar.current.date(byAdding: .day, value: -30, to: Date()) ?? Date()
        try await cleanupSyncMetadata(olderThan: thirtyDaysAgo)
    }
    
    /// Default implementation for compacting history with 7-day retention
    func compactSyncHistory<T: Syncable>(for entityType: T.Type) async throws {
        try await compactSyncHistory(for: entityType, keepDays: 7)
    }
}

// MARK: - Convenience Extensions

public extension Array where Element == SyncUploadResult {
    
    /// Filter successful uploads
    var successful: [SyncUploadResult] {
        return self.filter { $0.success }
    }
    
    /// Filter failed uploads
    var failed: [SyncUploadResult] {
        return self.filter { !$0.success }
    }
...
public extension Array where Element == SyncApplicationResult {
    
    /// Filter successful applications
    var successful: [SyncApplicationResult] {
        return self.filter { $0.success }
    }
    
    /// Filter failed applications
    var failed: [SyncApplicationResult] {
        return self.filter { !$0.success }
    }
    
    /// Filter applications with conflicts
    var withConflicts: [SyncApplicationResult] {
        return self.filter { $0.conflictDetected }
    }
...
public extension EntitySyncStatus {
    ...
    var needsAttention: Bool {
        return hasConflicts || lastError != nil
    }
}